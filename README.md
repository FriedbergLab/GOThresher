...
2. 2. Three files corresponds to the mapping between each GO_term and its ancestors in its own respective ontology
...
2. Run the program `debias_prep` program and provide the downloaded `obo` file. See the usage details at . This program needs to be run only when a new `obo` file needs to be used.
